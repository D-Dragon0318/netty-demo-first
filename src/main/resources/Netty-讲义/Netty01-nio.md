...
selector 的作用就是配合一个线程来管理多个 channel，获取这些 channel 上发生的事件，这些 channel 工作在非阻塞模式下，不会让线程吊死在一个 channel 上。适合**连接数特别多，但流量低**的场景（low traffic）
...
public class ChannelDemo1 {
    public static void main(String[] args) {
        ...
        try (RandomAccessFile file = new RandomAccessFile("helloword/data.txt", "rw")) {
            //用于io操作的通道（通道就像大船向码头伸出的甲板）
            FileChannel channel = file.getChannel();
            //分配10个字节的bytebuffer（10个字节是码头能容纳的数量）
            ByteBuffer buffer = ByteBuffer.allocate(10);
            do {
                // 向 buffer 写入，通道写到buffer缓冲区中
                ...
                // 切换 buffer 读模式
                buffer.flip();
                //判断是否有未读数据，可能是通过判断当前位置与已存入数据的位置
                while(buffer.hasRemaining()) {
                    //有则get出并转为char类型
                    //读取当前 position 位置的字节，并将 position 向前移动一位
                    log.debug("{}", (char)buffer.get());
                }
                // 切换 buffer 写模式
                /*clear() 方法底层主要是通过调整其内部维护的几个重要状态变量来实现的。这些状态变量包括：
                1.position（位置）：指示下一个要读或写的元素的索引。
                2.limit（限制）：指示缓冲区中可访问元素的数量。
                对于读操作，它代表了最后一个可读元素的下一个位置；对于写操作，它代表了最多能写入多少元素而不必增加缓冲区容量。
                3.capacity（容量）：缓冲区的最大容量，即它可以容纳的元素数量。*/
                //这里清空缓冲区，将指示数据最后位置的指针置初态
                buffer.clear();
            } while (true);
        } catch (IOException e) {
            e.printStackTrace();
...
clear() 适合于你想**丢弃缓冲区中所有**数据（包括未读数据），并重新开始填充缓冲区的情况。
compact() 适合于你**仍需要缓冲区中未读**的数据，并希望在这些数据之后追加新数据的情况，它能够有效地避免数据复制，提高效率。
