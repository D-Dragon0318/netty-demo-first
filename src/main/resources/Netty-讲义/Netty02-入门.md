...
![image-20240619152328604](img/image-20240619152328604.png)
...
* ctx.channel().write(msg) vs ctx.write(msg)
  ...
  * ctx.write(msg) 是从当前节点找**上一个**出站处理器
...
* 入站 ByteBuf 处理原则
  ...
  * 将原始 ByteBuf **转换**为其它类型的 Java 对象，这时 ByteBuf 就没用了，必须 **release**
  * 如果**不调用** ctx.fireChannelRead(msg) 向后传递，那么也必须 release
  * 注意各种**异常**，如果 ByteBuf 没有成功传递到下一个 ChannelHandler，必须 **release**
  * 假设消息一直向后传，那么 TailContext （尾）会负责释放**未处理消息**（原始的 ByteBuf）
* 出站 ByteBuf 处理原则
  * 出站消息最终都会转为 ByteBuf 输出，一直向前传，由 HeadContext（头部） flush 后 release
* 异常处理原则
