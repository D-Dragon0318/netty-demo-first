...
>   * 窗口内的数据才允许被发送，当应答未到达前，窗口**必须停止滑动**
...
>  MSS 限制
>
>  * 链路层对一次能够发送的最大数据有限制，这个限制称之为 MTU（maximum transmission unit）-- 最大传输单元，不同的链路设备的 MTU 值也有所不同，例如
>
>   * 以太网的 MTU 是 1500
>   * FDDI（光纤分布式数据接口）的 MTU 是 4352
>   * 本地回环地址的 MTU 是 65535 - 本地测试不走网卡
>
>  * MSS 是最大段长度（maximum segment size）-- 最大帧，它是 MTU 刨去 tcp 头和 ip 头后剩余能够作为数据传输的字节数
>    MTU = tcp头 + ip头 + MSS
>
>   * ipv4 tcp 头占用 20 bytes，ip 头占用 20 bytes，因此以太网 MSS 的值为 1500 - 40 = 1460
>   * TCP 在传递大量数据时，会按照 MSS 大小将数据进行分割发送
>   * MSS 的值在三次握手时通知对方自己 MSS 的值，然后在两者之间**选择一个小值**作为 MSS
>
...
> * 该算法是指发送端即使还有应该发送的数据，但如果这部分数据很少的话，则进行**延迟发送** （SO_SNDBUF 发送缓冲区)
>   * 如果 SO_SNDBUF 的数据达到 MSS，则需要发送
>   * 如果 SO_SNDBUF 中含有 FIN（表示需要**连接关闭**）这时将剩余数据发送，再关闭
>   * 如果 TCP_NODELAY = true，则需要发送 （紧急的需要即可发送）
>   * 已发送的数据**都收到 ack** 时，则需要发送
>   * 上述条件不满足，但发生**超时**（一般为 200ms）则需要发送
>   * 除上述情况，延迟发送
...
EmbeddedChannel 是 Netty 框架提供的一种特殊的 Channel 实现，主要用于**单元测试目的**。它允许你在内存中创建一个 **Channel 环境**，这样可以在不依赖网络IO的情况下，测试 ChannelHandler 的行为和处理逻辑。
使用 EmbeddedChannel 的好处包括：

* 简化测试：你可以在完全控制的环境中测试编码器（ChannelOutboundHandler）和解码器（ChannelInboundHandler），而无需设置真实的网络连接。
* 验证处理流程：可以通过写入消息到 Channel 并立即检查读取的结果来验证消息是否被正确地处理和转换。
* 检查 Channel 事件：可以验证 ChannelHandler 是否触发了预期的 Channel 事件，如通道激活、通道读取就绪等。
* 性能测试：可以在没有网络延迟和其他外部因素影响的情况下，测量消息处理的性能。
...
* 当 handler **不保存状态**时，就可以安全地在多线程下被共享
